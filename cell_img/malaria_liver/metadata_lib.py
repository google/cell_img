...
class MetadataIndex(object):
  ...
  def get_images_for_df(self, example_df: pd.DataFrame, patch_size: int,
                        name_for_x_col: str,
  ...
  def contact_sheet_for_df(self,
    ...
    raw_image_list = self.get_images_for_df(
        example_df,
...
class MetadataCache(object):
  ...
  def get_plate_uid(self, plate):
    ...
    """
    if plate not in self.cached_plate_uids.keys():
  ...
  def get_batch(self, plate_uid):
    ...
    Raises:
      ValueError: If the requested plate uid is not found.
    """
    if plate_uid not in self.cached_batches.keys():
      self.read_and_cache_plate_level()
  ...
  def get_plate_layout_int(self, plate_uid):
    ...
    Returns:
      plate_layout_int: Int representing the number of plates per row in
        Neuroglancer.
    """
    if plate_uid not in self.cached_plate_layout_ints.keys():
      self.read_and_cache_plate_level()
  ...
  def get_site_layout(self, plate_uid):
    ...
    """
    if plate_uid not in self.cached_site_layouts.keys():
      self.read_and_cache_plate_level()
  ...
  def get_well_layout(self, plate_uid):
    ...
    """
    if plate_uid not in self.cached_well_layouts.keys():
      self.read_and_cache_plate_level()
