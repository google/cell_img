...
def get_t2_statistic(
  ...
  control_emb_df = np.matmul(control_emb_df, np.array(control_eigenvectors))
  control_mean = jnp.mean(control_emb_df.to_numpy(), axis=0)
  other_emb_df = np.matmul(other_emb_df, np.array(control_eigenvectors))
  if scale_to_control_emb:
    other_emb_df = other_emb_df.div(
        control_emb_df.groupby('plate').std(), level='plate')
